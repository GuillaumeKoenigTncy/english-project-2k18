...
function quizActions(){
  ...
  function getThisQuest(direction){
    ...
    $('.btnNext').click(function(){
      ...
      // normal behavior
        ...
        //console.log(GlobalData[counter]);
        //counter ++;
  ...
  function sendResult(){
    ...
    var name = url.searchParams.get("name");
     ...
     window.location.href = "success.php?name="+name+"&category="+c;
  };
...
function buildQuiz(qID, qText, qAnswers, qLength, qCorrectAnswers){
  ...
  for(n=0; n < qAnswers.length; n++){
    ...
    if ((qAnswers[n].text) && (!qAnswers[n].image)) {
      midCode.push('<div class="col-xs-6 col-lg-12"><div class="quizBox" id="' + qAnswers[n].id + '">' + '<div class="ansText">' + qAnswers[n].text + '</div></div></div>');
    }
    // when answers are image-only
    else if ((qAnswers[n].image) && (!qAnswers[n].text)) {
      midCode.push('<div class="col-lg-3 col-xs-6"><div class="quizBox" id="' + qAnswers[n].id + '">' + '<div class="ansImage"><img src="' + qAnswers[n].image + '"/></div></div></div>');
    }
    // when answers are text and image
    else if ((qAnswers[n].image) && (qAnswers[n].text)) {
      midCode.push('<div class="col-lg-3 col-xs-6"><div class="quizBox" id="' + qAnswers[n].id + '">' + '<div class="ansImage"><img src="' + qAnswers[n].image + '"/>></div><div class="ansText">' + qAnswers[n].text + '</div></div></div>');
    }
  };
  midCodeFull = midCode.join(' ');
  // build HTML in parts
  var quizTop = '<section class="quizBody text-center"><h2 class="quizQuestion"><span class="questionNum">' + qID + '/' + qLength + '</span> ' + '<span class="questionTitle">'  + qText + '</span>' + '</h2><span class="quizAlert"> <i class="fas fa-exclamation-triangle"></i> This question requires an answer</span><div class="row">';
